        ...
        {isNavOpen && (
          ...
          >
            ...
            <div className="flex flex-col gap-4 md:gap-8 text-center">
              {[ { label: 'EXP', id: 'exp' }, { label: 'PROJECTS', id: 'projects' }, { label: 'ABOUT ME', id: 'about' }, { label: 'CONTACT', id: 'contact' } ].map((item, i) => (
                <motion.button
                  key={item.label}
      ...
      <nav className="fixed top-0 w-full z-[100] flex items-center justify-between px-1 sm:px-4 py-2 sm:py-4 md:py-8 mix-blend-difference">
        <div className="flex items-center gap-3 md:gap-3">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 md:w-10 md:h-10 bg-yellow-400 rounded-full flex items-center justify-center text-black font-black text-[10px] md:text-sm">AP</div>
            <span className="hidden md:flex lg:flex font-bold uppercase tracking-tight text-white text-xs md:text-base">Arnav</span>
          </div>
...
const ExperienceCard = ({ mediaType, mediaSrc, year, company, role, details, isDarkMode }: ExperienceCardProps) => (
  <div className="w-full flex-shrink-0 relative flex overflow-hidden shadow-xl transition-all duration-700 min-h-[350px] md:min-h-[500px] lg:min-h-[600px] max-h-[90vh] aspect-video">
    {mediaType === 'video' ? (
      <video src={mediaSrc} autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover object-center opacity-90 grayscale-[10%] z-0" style={{aspectRatio:'16/9'}} />
    ) : (
      <img src={mediaSrc} alt={company} className="absolute inset-0 w-full h-full object-contain object-center p-8 opacity-90 grayscale-[10%] z-0" style={{ aspectRatio: '16/9' }} />
    )}
